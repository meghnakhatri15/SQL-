...
-- window function (analytical function-- apply aggregate function based on current row)

-- over()-- specify the order of rows within each group.
...
-- partication by(seprated by smililar values)
...
-- rank(skip the rank for same values(1,2,2,3)
select *,
row_number() over(partition by department),
...
dense_rank() over(partition by department order by sale_amount)
from employee_sales;